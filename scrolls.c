...
void read_scroll(void)
{
    ...
    case S_CREATE:
    {
        // Create a monster: First look in a circle around him, next try his room otherwise give up
        int i = 0;
        ...
        {
            THING *newobj = new_item();
            new_monster(newobj, randmonster(false), &mp);
        }
        break;
    }
    case S_ID_POTION:
    ...
    case S_MAP:
        ...
        for (int y = 1; y < NUMLINES - 1; y++)
        {
            for (int x = 0; x < NUMCOLS; x++)
            {
                ...
                if (ch != ' ')
                {
                    THING *mobj = pp->p_monst;
                    if (mobj != NULL)
                    {
                        mobj->t_oldch = ch;
                    }
                    if (mobj == NULL || !on(player, SEEMONST))
                    {
                        mvaddch(y, x, ch);
                    }
                }
    ...
    case S_FDET:
    {
        ...
        bool found = false;
        wclear(hw);
        for (THING *fobj = lvl_obj; fobj != NULL; fobj = next(fobj))
        {
            if (fobj->o_type == FOOD)
            {
                found = true;
                wmove(hw, fobj->o_pos.y, fobj->o_pos.x);
                waddch(hw, FOOD);
            }
        }
        if (found)
