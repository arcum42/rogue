...
void death(char monst)
{
	...
	else
	{
		...
		snprintf(prbuf, 2 * MAXSTR, "%d Au", purse);
		move(15, center(prbuf));
		addstr(prbuf);
		snprintf(prbuf, 2 * MAXSTR, "%4d", 1900 + lt->tm_year);
	...
	fflush(stdout);
	(void)fgets(prbuf, 2 * MAXSTR, stdin);
	my_exit(0);
}
...
char *killname(char monst, bool doart)
{
	...
	const char *sp;
	bool article;
	static const struct h_list nlist[] = {
		{'a', "arrow", true},
		{'b', "bolt", true},
		{'d', "dart", true},
		{'h', "hypothermia", false},
	...
	if (isupper((unsigned char)monst))
	{
	...
	else
	{
		...
		article = false;
		for (hp = (struct h_list *)nlist; hp->h_ch; hp++)
			if (hp->h_ch == monst)
			{
				sp = hp->h_desc;
				article = hp->h_print;
	...
	if (doart && article)
		snprintf(prbuf, 2 * MAXSTR, "a%s ", vowelstr(sp));
	else
		prbuf[0] = '\0';
	strcat(prbuf, sp);
	return prbuf;
